...
public class AuthService {
    ...
    public ResponseEntity<String> join(String userId, String password, String userName, String userRole, String userMood) {
        //userName 중복 check
        try{
            ...
            User user = User.builder()
                    .userId(userId).userPassword(encoder.encode(password)).userName(userName).userRole(userRole).userMood(userMood).build();
    ...
    public ResponseEntity<String> login(String userId, String password) throws Exception {
        ...
        try {
            ...
            // token 유효시간 : 30분
            return ResponseEntity.ok().body(JwtUtil.IssuanceToken(userRepository.findByUserId(userId).get().getId(),key,1000L*60*30));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
        }
